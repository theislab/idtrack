...
def existing_outs_for_release(results_dir: Path,
    ...
    Preferred layout (assembly-aware):
    ...
    base_candidates = [
        results_dir / dataset_key / f"{assembly_label}_{release}",
        results_dir / dataset_key / assembly_label,   # just in case your layout is <dataset>/<assembly>/...
        results_dir / dataset_key,                    # fallback for older runs without assembly in path
    ]
    ...
    def _outs_is_finished(outs: Path) -> tuple[bool, float]:
        ...
        # Very weak fallback: any content under outs/
        if not found:
            try:
    ...
    for base in base_candidates:
        ...
        # Tags are generated by _mk_alignment_tag(release, cr_ver, prefix="GHRc") -> GHRc{release}_{MM}
        ...
        # If we found a finished run under an assembly-aware base, stop early
        if best_outs and base.name.startswith(assembly_label):
            break
...
def run_cellranger_master(
    *,
    ...
    if download_if_missing or not any(ds_dir.iterdir()):
        raise ValueError("download requested.", ds_dir)
        # try:
