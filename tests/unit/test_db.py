...
class TestDBHDF5Settings:
    ...
    def test_utf8_str_dtype(self):
        ...
        from idtrack._db import DB
        import h5py
        # Just verify it's an h5py special type
        assert DB.UTF8_STR is not None
...
class TestDBImmutability:
    ...
    def test_class_attributes_accessible(self):
        ...
        from idtrack._db import DB
        required_attrs = [
            "id_ver_delimiter", "first_version", "connection_timeout",
            "reading_timeout", "ensembl_ftp_base", "rest_server_api",
            "mysql_host", "assembly_mysqlport_priority", "main_assembly",
            "node_type_str", "nts_external", "forms_in_order",
            "backbone_form", "nts_ensembl", "connection_dict",
        ]
